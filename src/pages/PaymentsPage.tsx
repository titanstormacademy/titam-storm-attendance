    ...
    <>
      ...
      <Paper radius="lg" withBorder>
        ...
        <Box visibleFrom="md"><Table.ScrollContainer minWidth={820}><Table verticalSpacing="sm" horizontalSpacing="lg" highlightOnHover><Table.Thead><Table.Tr><Table.Th>Student</Table.Th><Table.Th>Monthly fee</Table.Th><Table.Th>Status</Table.Th><Table.Th>Received</Table.Th><Table.Th>Method / reference</Table.Th><Table.Th>Outstanding</Table.Th><Table.Th /></Table.Tr></Table.Thead><Table.Tbody>{activeEnrolled.map((student) => {
          ...
          return <Table.Tr key={student.id} onClick={() => showHistory(student)} style={{ cursor: 'pointer' }}><Table.Td><Text fw={700}>{student.name}</Text><Text size="xs" c="dimmed">{student.parent_contact || student.student_phone || 'No contact'}</Text></Table.Td><Table.Td>RM {money(student.monthly_fee || 0)}</Table.Td><Table.Td><StatusBadge status={best?.status} /></Table.Td><Table.Td><Text fw={650}>RM {money(total)}</Text><Text size="xs" c="dimmed">{best?.date_received ? dayjs(best.date_received).format('D MMM YYYY') : '—'}</Text></Table.Td><Table.Td><Text size="sm">{best?.method || '—'}</Text><Text size="xs" c="dimmed">{best?.reference_no || 'No reference'}</Text></Table.Td><Table.Td><Text fw={650}>{due?.months.length || 0} months</Text><Text size="xs" c="dimmed">RM {money(due?.amount || 0)}</Text></Table.Td><Table.Td><Button variant="light" size="xs" onClick={(event) => { event.stopPropagation(); openNew(student) }}>Add</Button></Table.Td></Table.Tr>
...
function PaymentCard({ student, payments, arrears, onHistory, onAdd }: { student: Student; payments: Payment[]; arrears?: { months: string[]; amount: number }; onHistory: () => void; onAdd: () => void }) {
  ...
  return <Paper p="md" radius="md" withBorder><Group justify="space-between" align="flex-start"><Box><Text fw={800}>{student.name}</Text><Text size="xs" c="dimmed">RM {money(student.monthly_fee || 0)} / month</Text></Box><StatusBadge status={best?.status} /></Group><SimpleGrid cols={2} mt="md"><Box><Text size="xs" c="dimmed">Received</Text><Text fw={700}>RM {money(total)}</Text></Box><Box><Text size="xs" c="dimmed">Outstanding</Text><Text fw={700}>{arrears?.months.length || 0} months</Text><Text size="xs" c="dimmed">RM {money(arrears?.amount || 0)}</Text></Box></SimpleGrid><Group grow mt="md"><Button variant="light" leftSection={<IconHistory size={16} />} onClick={onHistory}>History</Button><Button leftSection={<IconPlus size={16} />} onClick={onAdd}>Add</Button></Group></Paper>
}
