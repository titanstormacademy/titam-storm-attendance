...
export function StudentsPage({ branchId, data, isAdmin, createRequest, onCreateHandled, onChanged }: { branchId: number; data: BootstrapData; isAdmin: boolean; createRequest: number; onCreateHandled: () => void; onChanged: () => Promise<unknown> }) {
  ...
  return <>
    ...
    {filtered.length ? <Box className="student-directory-grid">{filtered.map((student) => {
      ...
      return <Paper key={student.id} role={isAdmin ? 'button' : undefined} tabIndex={isAdmin ? 0 : undefined} className={`student-directory-card ${isAdmin ? '' : 'read-only'}`} p="sm" radius="lg" withBorder onClick={isAdmin ? () => showProfile(student) : undefined} onKeyDown={isAdmin ? (event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); showProfile(student) } } : undefined}><PersonAvatar name={student.name} src={src} size={68} onClick={!isAdmin ? () => setPhotoView({ src, name: student.name }) : undefined} /><Text className="student-directory-name" fw={700} size="sm" mt="xs" lineClamp={2}>{student.name}</Text><Text size="xs" c="dimmed" mt={3}>{enrollmentCount ? `${enrollmentCount} class${enrollmentCount > 1 ? 'es' : ''}` : 'No class'}</Text><Badge mt="xs" size="sm" variant="light" color={statusColor(student.status)}>{student.status}</Badge></Paper>
      ...
      <Stack>
        ...
        <Tabs defaultValue="profile">
          ...
          <Tabs.Panel value="profile" pt="md"><Grid gutter="sm" className="student-profile-form">
            ...
            <Grid.Col span={{ base: 12, xs: 4 }}><Select label="Status" value={form.status} disabled={!isAdmin} onChange={(value) => setForm({ ...form, status: value as Student['status'] })} data={['Active', 'Trial', 'Inactive']} /></Grid.Col>
            ...
            <Grid.Col span={{ base: 12, xs: 3 }}><Select label="Gender" value={form.gender} disabled={!isAdmin} onChange={(value) => setForm({ ...form, gender: (value || '') as Student['gender'] })} data={['Male', 'Female']} clearable /></Grid.Col>
            <Grid.Col span={{ base: 12, xs: 4 }}><TextInput label="Date of birth" type="date" value={form.date_of_birth || ''} disabled={!isAdmin} onChange={(event) => setForm({ ...form, date_of_birth: event.currentTarget.value || null })} /></Grid.Col>
            <Grid.Col span={{ base: 12, xs: 6 }}><TextInput label="Student phone" type="tel" value={form.student_phone || ''} disabled={!isAdmin} onChange={(event) => setForm({ ...form, student_phone: event.currentTarget.value })} /></Grid.Col>
            <Grid.Col span={{ base: 12, xs: 6 }}><TextInput label="School" value={form.school || ''} disabled={!isAdmin} onChange={(event) => setForm({ ...form, school: event.currentTarget.value })} /></Grid.Col>
            ...
            <Grid.Col span={{ base: 12, xs: 4 }}><Select label="Level" value={form.level} disabled={!isAdmin} onChange={(value) => setForm({ ...form, level: (value || '') as Student['level'] })} data={['Beginner', 'Intermediate', 'Advanced']} clearable /></Grid.Col>
