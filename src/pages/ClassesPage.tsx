...
export function ClassesPage({ branchId, data, onChanged }: { branchId: number; data: BootstrapData; onChanged: () => Promise<unknown> }) {
  ...
  function edit(item?: AcademyClass) { setForm(item ? { ...item } : { ...blankClass }); formModal.open() }

  async function submit() {
    if (!form.label.trim()) return
    setSaving(true)
    try { await saveClass(branchId, form); notifications.show({ color: 'green', message: 'Class saved' }); formModal.close(); await onChanged() } catch (error) { notifications.show({ color: 'red', message: errorMessage(error) }) } finally { setSaving(false) }
  }

  async function remove(item: AcademyClass) {
    if (!window.confirm(`Delete ${item.label}, its sessions, enrollments, attendance, and coach attendance?`)) return
    try { await deleteClass(item.id); await onChanged() } catch (error) { notifications.show({ color: 'red', message: errorMessage(error) }) }
  }
  ...
  async function openSessions(item: AcademyClass) {
    setTarget(item); sessionsModal.open(); setSaving(true)
    try { setSessions(await getClassSessions(item.id)) } catch (error) { notifications.show({ color: 'red', message: errorMessage(error) }) } finally { setSaving(false) }
  }

  function openSessionForm(session?: Session) {
    setSessionForm(session ? { id: session.id, session_date: session.session_date, notes: session.notes || '', coach_id: session.coach_id } : { session_date: dayjs().format('YYYY-MM-DD'), notes: '', coach_id: target?.coach_id || null })
    sessionFormModal.open()
  }

  async function submitSession() {
    if (!target || !sessionForm.session_date) return
    setSaving(true)
    try {
      await saveSession({ ...sessionForm, branch_id: branchId, class_id: target.id })
      notifications.show({ color: 'green', message: sessionForm.id ? 'Session updated' : 'Session created' })
      sessionFormModal.close(); setSessions(await getClassSessions(target.id)); await onChanged()
    } catch (error) { notifications.show({ color: 'red', message: errorMessage(error) }) } finally { setSaving(false) }
  }
  ...
  return (
    <>
      <PageHeader title="Classes" description="Weekly schedules, coach assignments, dated sessions, and one-off training" action={<Button leftSection={<IconPlus size={17} />} onClick={() => edit()}>Add class</Button>} />
      ...
      <Modal opened={formOpened} onClose={formModal.close} title={form.id ? 'Edit class' : 'Add class'} centered><Stack><TextInput label="Class name" placeholder="Saturday Morning Elite" value={form.label} onChange={(event) => setForm({ ...form, label: event.currentTarget.value })} required /><Select label="Day of week" data={days} value={form.day_of_week} onChange={(value) => setForm({ ...form, day_of_week: value || 'Saturday' })} /><Grid><Grid.Col span={{ base: 12, xs: 6 }}><TextInput label="Start time" type="time" value={form.start_time || ''} onChange={(event) => setForm({ ...form, start_time: event.currentTarget.value })} /></Grid.Col><Grid.Col span={{ base: 12, xs: 6 }}><TextInput label="End time" type="time" value={form.end_time || ''} onChange={(event) => setForm({ ...form, end_time: event.currentTarget.value })} /></Grid.Col></Grid><Select label="Head coach" clearable value={form.coach_id ? String(form.coach_id) : null} onChange={(value) => setForm({ ...form, coach_id: value ? Number(value) : null })} data={data.coaches.filter((coach) => coach.status === 'Active' && coach.coach_type === 'Head').map((coach) => ({ value: String(coach.id), label: coach.name }))} /><Group className="modal-actions" justify="flex-end"><Button variant="default" onClick={formModal.close}>Cancel</Button><Button onClick={submit} loading={saving}>Save class</Button></Group></Stack></Modal>

      <Modal opened={generateOpened} onClose={generateModal.close} title={`Generate ${target?.label || ''} sessions`} centered><Stack><Text size="sm" c="dimmed">Creates one session every {target?.day_of_week} and skips dates that already exist.</Text><Grid><Grid.Col span={{ base: 12, xs: 6 }}><TextInput label="From" type="date" value={fromDate} onChange={(event) => setFromDate(event.currentTarget.value)} /></Grid.Col><Grid.Col span={{ base: 12, xs: 6 }}><TextInput label="To" type="date" value={toDate} onChange={(event) => setToDate(event.currentTarget.value)} /></Grid.Col></Grid><Button leftSection={<IconCalendarPlus size={17} />} onClick={runGenerator} loading={saving}>Generate sessions</Button></Stack></Modal>

      <Modal opened={sessionsOpened} onClose={sessionsModal.close} title={`${target?.label || ''} sessions`} size="lg" centered><Stack><Group justify="space-between"><Text c="dimmed" size="sm">Add one-off training or correct generated sessions.</Text><Button leftSection={<IconPlus size={16} />} onClick={() => openSessionForm()}>Add session</Button></Group>{sessions.length ? sessions.map((session) => <Paper key={session.id} p="md" radius="md" withBorder><Group justify="space-between" align="flex-start"><Box><Text fw={750}>{dayjs(session.session_date).format('dddd, D MMMM YYYY')}</Text><Text size="sm" c="dimmed">{session.coach?.name || 'No coach'}{session.notes ? ` · ${session.notes}` : ''}</Text></Box><Group gap="xs"><ActionIcon aria-label="Edit session" size={44} variant="light" onClick={() => openSessionForm(session)}><IconEdit size={17} /></ActionIcon><ActionIcon aria-label="Delete session" size={44} variant="light" color="red" onClick={() => removeSession(session)}><IconTrash size={17} /></ActionIcon></Group></Group></Paper>) : <Text c="dimmed" ta="center" py="xl">No sessions yet.</Text>}</Stack></Modal>

      <Modal opened={sessionFormOpened} onClose={sessionFormModal.close} title={sessionForm.id ? 'Edit session' : 'Add session'} centered><Stack><TextInput label="Date" type="date" value={sessionForm.session_date} onChange={(event) => setSessionForm({ ...sessionForm, session_date: event.currentTarget.value })} required /><Select label="Coach" clearable value={sessionForm.coach_id ? String(sessionForm.coach_id) : null} onChange={(value) => setSessionForm({ ...sessionForm, coach_id: value ? Number(value) : null })} data={data.coaches.filter((coach) => coach.status === 'Active').map((coach) => ({ value: String(coach.id), label: coach.name }))} /><Textarea label="Notes" value={sessionForm.notes} onChange={(event) => setSessionForm({ ...sessionForm, notes: event.currentTarget.value })} /><Group className="modal-actions" justify="flex-end"><Button variant="default" onClick={sessionFormModal.close}>Cancel</Button><Button onClick={submitSession} loading={saving}>Save session</Button></Group></Stack></Modal>
    </>
  )
}
