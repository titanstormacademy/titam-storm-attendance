...
import { Alert, Badge, Box, Button, Group, Paper, SegmentedControl, Select, Stack, Table, Text } from '@mantine/core'
...
export function ReportsPage({ branchId, data }: { branchId: number; data: BootstrapData }) {
  ...
  return <>
    <PageHeader title="Reports" description="Student attendance overview" action={<Button variant="light" leftSection={<IconClipboard size={17} />} onClick={copyReport} disabled={loading || Boolean(loadError)}>Copy</Button>} />
    ...
    {loading ? <PageLoader label="Loading report…" /> : loadError ? <Alert color="red" title="Could not load report">{loadError}<Button mt="md" size="xs" onClick={() => setReloadRequest((current) => current + 1)}>Retry</Button></Alert> : rows.length === 0 ? <Paper p="xl" radius="lg" withBorder><Text c="dimmed" ta="center">No attendance records for {period.label}.</Text></Paper> : view === 'summary' ? <Stack gap="sm">{rows.map((row) => {
      const expanded = expandedRows.has(row.id)
      return <Paper key={row.id} className="report-student-summary" radius="lg" withBorder><Box component="button" type="button" className="report-summary-toggle" aria-expanded={expanded} onClick={() => toggleRow(row.id)}><Group justify="space-between" align="flex-start" wrap="nowrap"><Box><Text fw={800}>{row.name}</Text><Text size="xs" c="dimmed">{row.enrolledClasses.length} class{row.enrolledClasses.length === 1 ? '' : 'es'} enrolled</Text></Box><Group gap="sm" wrap="nowrap"><Box ta="center"><Text className="report-total">{row.total}</Text><Text size="xs" c="dimmed">attendances</Text></Box><IconChevronDown className={expanded ? 'expanded' : ''} size={20} /></Group></Group><Group gap={5} mt="xs">{row.enrolledClasses.map((item) => <Badge key={item.id} size="sm" variant="light" color="blue">{item.label}</Badge>)}</Group></Box>{expanded && <Box className="report-summary-detail"><DividerLine /><Group justify="space-between" mb="sm"><Text fw={750} size="sm">{period.label}</Text><Text fw={750} size="sm">{row.total} attendances</Text></Group>{row.classes.size ? <Stack gap="md">{[...row.classes].map(([classId, item]) => <Box key={classId}><Group justify="space-between" wrap="nowrap"><Group gap="xs" wrap="nowrap"><span className="report-class-dot" style={{ background: colorFor(classId) }} /><Text size="sm" c="dimmed">{item.label}{item.walkIns ? ` · ${item.walkIns} walk-in` : ''}</Text></Group><Text size="sm" fw={700}>{item.dates.length}</Text></Group><Stack gap={4} mt={5} ml="md">{item.dates.sort().map((attendanceDate) => <Group key={attendanceDate} gap="xs"><IconCheck size={14} color="#22c55e" /><Text size="xs" c="dimmed">{dayjs(attendanceDate).format('D MMM YYYY')}</Text></Group>)}</Stack></Box>)}</Stack> : <Text size="sm" c="dimmed">No attendance in this period.</Text>}</Box>}</Paper>
    })}</Stack> : <>
